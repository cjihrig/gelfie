...
const supportedTransports = new Set(['udp', 'tcp']);
...
class GelfClient extends EventEmitter {
  constructor (options = {}) {
    ...
    const {
      host = Os.hostname(),
      serializer = GelfClient.defaultSerializer,
      transport = 'udp',
    ...
    if (transport === 'udp') {
      ...
      this[kTransport] = new TcpTransport(options, this);
    }
  }
  ...
  [kLogFunction] (short, full, additionalFields, timestamp, level) {
    ...
    const message = {
      ...
      host: this.host,
      short_message: this.serializer(short),
      full_message: full === undefined ? full : this.serializer(full),
  ...
  static defaultSerializer (value) {
    ...
    const v = !types.isNativeError(value) ? value : {
      name: value.name,
